...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val colorScheme = when (themeType.value) {
                ThemeType.PURPLE -> {
                    if (darkTheme.value) DarkPurpleColors
                    else
                        LightPurpleColors
                }

                ThemeType.RED -> {
                    if (darkTheme.value) DarkRedColors else
                        LightRedColors
                }

                ThemeType.GREEN -> {
                    if (darkTheme.value) DarkGreenColors
                    else
                        LightGreenColors
                }
    ...
    fun MainScreen(
        ...
        val currentRoute =
            currentNavBackStackEntry?.destination?.route ?: Destinations.Pantalla1.route
        ...
        ModalNavigationDrawer(
           ...
           navController =
            navController)
                ...
                NavigationHost(
                    navController, darkMode, modif = it
                )
            }
        }
